...
function _opened() {
  let i;
  console.log(_subscribeList);

  if(_lost && _subscribeList.length > 0){
      _queue = _subscribeList.map(a => Object.assign({}, a));
  }
...
function _defer(context, type, name, callback, args, kwArgs, options) {
  ...
  if (!_session) {
    i.defer = defer();
    _queue.push(i);
    if(i.type === 'subscribe' && typeof _subscribeList.find(o => o.name === i.name) === 'undefined') _subscribeList.push(i);
